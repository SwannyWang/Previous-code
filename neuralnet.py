...
#only shuffle once every epoch. Shuffle training as well.
...
class SoftMaxCrossEntropy:
    ...
    def _softmax(self, z: np.ndarray) -> np.ndarray:
        ...
        """
        # TODO: implement
        #raise NotImplementedError ?z or b?
    ...
    def backward(self, y: int, y_hat: np.ndarray) -> np.ndarray:
        ...
        #raise NotImplementedError dJ/db?
        real_y = np.zeros(shape=len(y_hat))
        real_y[y] = 1
        return y_hat-real_y
...
class Sigmoid:
    ...
    def backward(self, dz: np.ndarray) -> np.ndarray:
        ...
        z2 = self.z*self.z
        z_prime = self.z-z2
        #print(self.z,z2,z_prime)
        djda = []
        for i in range(dz.shape[0]):
            djda.append(dz[i]*(z_prime[i]))
        #print(djda)
        return djda
...
class Linear:
    def __init__(self, input_size: int, output_size: int,
        ...
        # raise NotImplementedError
        # bias = np.zeros(shape = [output_size,1])
    ...
    def backward(self, dz: np.ndarray) -> np.ndarray:
        ...
        #print(dz)
        self.djdw = np.dot(dz,self.x)
        #print(self.djdw)
        return np.dot(self.weights[:,1:].transpose(), dz)  # dJdx/dJdz
    ...
    def step(self) -> None:
        ...
        # TODO: implement
        # raise NotImplementedError
        self.weights -= self.lr * self.djdw
        # print(self.weights_new)
...
class NN:
    ...
    def forward(self, x: np.ndarray, y: int) -> Tuple[np.ndarray, float]:
        ...
        y_hat = self.softmaxcrossentropy.forward(b, y)[0]
        J = self.softmaxcrossentropy.forward(b, y)[1]
        # print(b)
        return y_hat, J
    ...
    def backward(self, y: int, y_hat: np.ndarray) -> None:
        ...
        # raise NotImplementedError
        djdb = self.softmaxcrossentropy.backward(y, y_hat)  # linear
        djdz = self.linear2.backward(djdb)
        djda = self.sigmoid.backward(djdz)
        djdx = self.linear1.backward(djda)
        # return djdb, djdz, djda, djdx
    ...
    def compute_loss(self, X: np.ndarray, y: np.ndarray) -> float:
        ...
        n = X.shape[0]
        for i in range(n):
            sum_loss += self.forward(X[i], y[i])[1]
        return sum_loss / n
    ...
    def train(self, X_tr: np.ndarray, y_tr: np.ndarray,
        ...
        train_losses = []
        test_losses = []
        # alpha = self.linear1.weights
        # beta = self.linear2.weights
        for i in range(n_epochs):
            X_tr_new, y_tr_new = shuffle(X_tr, y_tr, i)
            for j in range(len(y_tr)):
                y_hat, J = self.forward(X_tr_new[j], y_tr_new[j])  # Get y_hat and J
                # gb, gz, ga, gx = self.backward(y_tr_new[j], y_hat) #Get dJdb, dJdz, djda, djdx
                self.backward(y_tr_new[j], y_hat)
                self.step()  # Update beta
            train_losses.append(self.compute_loss(X_tr_new, y_tr_new))
    ...
    def test(self, X: np.ndarray, y: np.ndarray) -> Tuple[np.ndarray, float]:
        ...
        err = 0
        for i in range(N):
            predicted = self.forward(X[i], y[i])[0]
            # print(predicted)
            y_hat.append(np.argmax(predicted))
        for j in range(N):
            if y_hat[j] != y[j]:
                err += 1
        return y_hat, err / N
