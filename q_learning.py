...
if __name__ == "__main__":
    ...
    slength = env.state_space
    alength = env.action_space
    W = np.zeros((alength, slength + 1))
    rewards = []
    ...
    for episode in range(episodes):
        ...
        for iteration in range(max_iterations):
            ...
            #       the bias term!
            Q_value = max(Q(W, state_))
            A = np.zeros((alength, slength + 1))
            A[action] = np.insert(init, 0, 1)
            W = W - lr * (q_ - (reward_ + gamma * Q_value)) * A
            init = state_
